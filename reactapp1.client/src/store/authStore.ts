...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: async (email, password, rememberMe) => {
    try {
      set({ loading: true, error: undefined });
      const res = await postApiRaw('account/login', { email, password, rememberMe });
      if (!res.ok) {
        set({ error: extractIdentityErrors(res.bodyJson) || res.bodyText || `Login failed (${res.status})` });
        return false;
      }
      await get().loadMe();
      return true;
  ...
  register: async (email, password) => {
    try {
      ...
      if (!res.ok) {
        set({ error: extractIdentityErrors(res.bodyJson) || res.bodyText || `Registration failed (${res.status})` });
        return false;
      }
