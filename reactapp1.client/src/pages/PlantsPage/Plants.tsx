...
export default function Plants() {
  ...
  useEffect(() => {
    loadSaved() /* reload when user changes */
  }, [favKey])
  ...
  const normalizeImageUrl = (raw?: string) => {
    ...
    // Convert Wikipedia/Wikimedia media-page links to direct file paths.
    // Example: https://en.wikipedia.org/wiki/Topic#/media/File:Some_Image.jpg
    // Result:  https://en.wikipedia.org/wiki/Special:FilePath/Some_Image.jpg
    if (/wikipedia\.org|wikimedia\.org/i.test(value)) {
      const mediaFileMatch = value.match(/#\/media\/File:([^?#]+)/i)
      if (mediaFileMatch?.[1]) {
        const fileName = mediaFileMatch[1]
        try {
          const parsed = new URL(value)
          return `${parsed.protocol}//${parsed.host}/wiki/Special:FilePath/${encodeURIComponent(fileName)}`
        } catch {
    ...
    if (/dropbox\.com/i.test(value)) {
      if (value.includes("?")) {
        value = value.replace(/([?&])dl=0/i, "$1raw=1")
        if (!/[?&]raw=1/i.test(value)) {
          value = `${value}&raw=1`
        }
      } else {
        value = `${value}?raw=1`
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        {isAdmin() && (
          <button className={formStyle.button} onClick={addHandler}>
            Pridėti augalą
          </button>
        )}
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {isSignedIn && (
            <button
              type="button"
              className={formStyle.button}
              style={onlyMine ? { backgroundColor: "#047857" } : undefined}
      ...
      <PlantChatbot plants={plants} visiblePlants={visiblePlants} />
          ...
          <table className="min-w-full table-auto">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {visiblePlants.map((plant) => (
                (() => {
                  const imageSrc = normalizeImageUrl(plant.imageUrl)
                  const imageVisible =
                    !!imageSrc && !brokenImageIds.has(plant.id ?? -1)
