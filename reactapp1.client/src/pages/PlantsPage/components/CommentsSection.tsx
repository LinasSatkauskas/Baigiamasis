...
export function CommentsSection({ plantId, plantName }: Props) {
    ...
    const load = () => getApi<IComment[]>(`comments?plantId=${plantId}`).then(c => c && setComments(c));

    useEffect(() => { load(); }, [plantId]);
    useEffect(() => { setValue("plantId", plantId); reset(edit ?? defaults); }, [edit, reset, defaults, plantId, setValue]);
    ...
    const onSubmit = handleSubmit(async (data, e) => {
        ...
        setSubmitting(true);
        try {
            const nativeSubmitter = (e?.nativeEvent as SubmitEvent | undefined)?.submitter as HTMLButtonElement | undefined;
    ...
    // Only Users/Admins see comments at all
                        ...
                        <table className="min-w-full table-auto">
                            <thead>
                                <tr className="bg-gray-50">
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Vartotojo paštas</th>
                                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Komentaras</th>
            ...
            {canPost ? (
                <form onSubmit={onSubmit} className="mt-6">
                    <div className="rounded-lg border border-gray-200 bg-white shadow p-4 flex flex-col gap-3">
                        ...
                        <input type="hidden" {...register("plantId")} /> {/* NEW */}

                        <div className="text-xs text-gray-600">
                            Rašysite kaip: <span className="font-medium text-gray-800">{currentEmail}</span>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                            <button className={formStyle.button} type="submit" name="action" value="update" disabled={submitting}>
                                {submitting ? "Išsaugoma..." : "Išsaugoti"}
                            </button>
                            <button className={formStyle.button} type="submit" name="action" value="new" disabled={submitting}>
                                {submitting ? "Kuriama..." : "Naujas"}
                            </button>
                            <button className={formStyle.button} type="submit" name="action" value="delete" disabled={submitting}>
