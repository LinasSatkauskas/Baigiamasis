...
export function PlantForm({ storePlant, plant, deletePlant }: Props) {
  ...
  useEffect(() => {
    ...
    if (plant?.pests && allPests.length > 0) {
      ...
      const ids = names
        .map(
          (n) =>
            allPests.find((p) => p.name.toLowerCase() === n.toLowerCase())?.id,
        )
    ...
    if (plant?.soilType && soils.length > 0) {
      ...
      const ids = names
        .map(
          (n) =>
            soils.find((s) => s.name.toLowerCase() === n.toLowerCase())?.id,
        )
        .filter((id): id is number => typeof id === "number")
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-3">
      ...
      <div>
        <label htmlFor="description" className={formStyle.label}>
          Aprašymas
        </label>
        <textarea
          id="description"
      ...
      <div>
        <label htmlFor="soilSelect" className={formStyle.label}>
          Dirvožemiai
        </label>
        <select
          id="soilSelect"
      ...
      <div>
        <label htmlFor="pestSelect" className={formStyle.label}>
          Kenkėjai
        </label>
        <select
          id="pestSelect"
      ...
      <div>
        <label htmlFor="pestControlMethod" className={formStyle.label}>
